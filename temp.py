...
class App:
    ...
    def capture_image(self):
        ...
        self.setup_coordinate_button = util.get_button(self.capture_image_window, 'Setup Coordinates', 'blue', self.setup_coordinates)
        self.setup_coordinate_button.place(x=750, y=200)
    ...
    def setup_coordinates(self):
        ...
        point1, point2 = None, None  # Reset the points

        self.coordinate_window = tk.Toplevel(self.main_window)
        self.coordinate_window.geometry("800x600+370+120")

        img_bgr = cv2.cvtColor(self.most_recent_capture_arr, cv2.COLOR_RGB2BGR)
        ...
        def on_click(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                if point1 is None:
                    point1 = (x, y)
                    cv2.putText(img_bgr, str(x) + ',' + str(y), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                    cv2.imshow('Setup Coordinates', img_bgr)
                else:
                    point2 = (x, y)
                    w = point2[0] - point1[0]
                    h = point2[1] - point1[1]
                    text = f"{point1[0]}, {point1[1]}, {w}, {h}"
                    cv2.putText(img_bgr, text, (point2[0], point2[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.imshow('Setup Coordinates', img_bgr)
                    print(text)
                    point1 = None
                    point2 = None
                    cv2.destroyWindow('Setup Coordinates')

        cv2.imshow('Setup Coordinates', img_bgr)
        cv2.setMouseCallback('Setup Coordinates', on_click)
        cv2.waitKey(0)
    ...
    def save_to_excel(self, detected_text):
        try:
            ...
            # Check if the detected weight is within the specified range for the selected product
            ...
            messagebox.showinfo("Success", "Detected text saved successfully!")
...
def extract_and_detect(img_path, db_path):
    ...
    # Coordinates based on image name
